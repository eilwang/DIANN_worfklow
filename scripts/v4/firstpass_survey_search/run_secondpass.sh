...
EMPLIB_TAG="$(basename "${EMPLIB}" .empirical.parquet)"

# Create directories if they don't exist
mkdir -p "${OUT_DIR}/second_pass/logs"
# mkdir -p "${OUT_DIR}/second_pass/quant" # not needed since first pass already has reusable quants
# mkdir -p "${OUT_DIR}/second_pass/empirical_library" # not need since we're not generated anthe rlib
